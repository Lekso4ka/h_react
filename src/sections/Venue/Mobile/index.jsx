...
export const Mobile = ({ data }) => {
    ...
    const ref = useRef();
    useEffect(() => {
        const plan = ref.current;
        if (plan) {
            const svg = plan.firstElementChild;
            const width = plan.getBoundingClientRect().width;
            setW(true)
            if (data.plan_rotate) {
    ...
    return <Content>
        <h1>{ data.name }</h1>
        <Images cnt={ data.images.length }>
            { data.images.map((el, i) => <Image key={ i } bg={ el }>{ i === 0 && <Tour
                dark
                link={ data.tour_link }
                pos/>
            }</Image>) }
