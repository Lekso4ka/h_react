...
async function readLocalized(fileName, lang) {
  const data = await readJsonLang(fileName, lang);
...
function createResourceRouter({ fileName, kind, idField = null }) {
  ...
  if (kind === "singleton") {
    router.get("/", async (req, res) => {
      try {
        const data = await readLocalized(fileName, langFromQuery(req));
        res.json({ kind, data });
      } catch (error) {
    ...
    router.put("/", async (req, res) => {
      try {
        ...
        if (lang === "en") {
          const ruItem = await readJsonLang(fileName, "ru");
          const synced = syncLocale(ruItem, item);
          await writeJsonLang(fileName, "en", synced);
          return res.json({ item: synced });
        }

        await writeJsonLang(fileName, "ru", item);
        const enItem = await readJsonLang(fileName, "en");
        await writeJsonLang(fileName, "en", syncLocale(item, enItem));
        res.json({ item });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
  ...
  router.post("/", async (req, res) => {
    try {
      ...
      if (kind === "object") {
        ...
        data[id] = item;
        await writeJsonLang(fileName, lang, data);

        const altLang = otherLang(lang);
        const altData = await readJsonLang(fileName, altLang);
        if (!altData[id]) {
          altData[id] = cloneJson(item);
          await writeJsonLang(fileName, altLang, altData);
        }
  ...
  router.put("/:id", async (req, res) => {
    try {
      ...
      if (kind === "object") {
        const ruData = lang === "ru" ? data : await readJsonLang(fileName, "ru");
        const enData = lang === "en" ? data : await readJsonLang(fileName, "en");
        ...
        if (lang === "en") {
          ...
          enData[id] = synced;
          await writeJsonLang(fileName, "en", enData);
          return res.json({ id, item: synced });
        }

        await writeJsonLang(fileName, "ru", ruData);
      ...
      if (lang === "en") {
        const ruData = await readJsonLang(fileName, "ru");
        ...
        data[index] = synced;
        await writeJsonLang(fileName, "en", data);
        return res.json({
      ...
      data[index] = item;
      await writeJsonLang(fileName, "ru", data);

      const enData = await readJsonLang(fileName, "en");
